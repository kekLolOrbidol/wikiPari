...
class GamePresenterImplementation @Inject constructor() : GamePresenter {
    ...
    @Inject
    lateinit var gameControll: GameControll
    ...
    override fun answerClicked(question: Question, duration: Long, article: Article?) {
        ...
        val gameState: Pair<Boolean, Boolean> = gameControll.checkWinState(question, duration, article)

        if (!gameState.second) {
            gameView.showAnswer(gameState.first, gameControll.currentQuestions[0])
        } else {
            gameView.gameOver(gameState.first, gameControll.currentHighscore)
        }
    }
    ...
    override fun startGame() {
        ...
        compositeDisposable.add(gameRepository.getRandomWikipediaArticlesForGame((10 * ANSWER_POSSIBILITIES) + EXTRA_IMAGE_THRESHOLD)
                ...
                .flatMap {
                    ...
                    val questions = gameControll.setupAndStartGame(it)
                    var ids = ""
                    questions.forEach {
                    ...
                    gameControll.setSolutionExtracts(it)
                }
                .subscribeWith(object : DisposableObserver<Any>() {
                    ...
                    override fun onNext(articles: Any) {
                        ...
                        gameView.displayNextQuestion(gameControll.currentQuestions[0])
                    }
                }))
    }
