...
import at.sunilson.wiki_quiz.repository.retrofitGsonEntities.WikiArticles
...
class GameRepository @Inject constructor() {
    ...
    fun getRandomWikipediaArticlesForGame(amount: Int): Observable<List<Article>> {
        return wikiService.getRandomArticlesForGame(amount).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).map {
            val wikiArticles: WikiArticles = Gson().fromJson(it.string(), WikiArticles::class.java)
            val results: MutableList<Article> = mutableListOf()

            wikiArticles.query.pages.forEach {
    ...
    fun getExtractsForArticles(pageids: String): Observable<Map<String, String>> {
        ...
        return wikiService.getExtractsForArticles(pageids).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).map {
            val wikiArticles: WikiArticles = Gson().fromJson(it.string(), WikiArticles::class.java)
